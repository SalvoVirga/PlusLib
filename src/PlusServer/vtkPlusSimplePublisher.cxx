...
//------------------------------------------------------------------------------
PlusStatus vtkPlusSimplePublisher::Start(vtkPlusDataCollector* dataCollector, vtkPlusTransformRepository* transformRepository, vtkXMLDataElement* serverElement, const std::string& configFilePath)
{
...
PlusStatus vtkPlusSimplePublisher::ReadConfiguration(vtkXMLDataElement* serverElement, const std::string& aFilename)
{
  ...
  if (defaultClientInfo != nullptr)
  {
    ...
    { LOG_ERROR("The wrong message type or none has been specified in the config file. IMAGE and TRANSFORM are the available message types.");
        return PLUS_FAIL;
    }
  }
  return PLUS_SUCCESS;
...
{
  vtkPlusSimplePublisher* self = (vtkPlusSimplePublisher*)(data->UserData);
  ...
  if (aChannel == nullptr)
  {
    LOG_WARNING("There are no channels to publish.");
  }
...
//----------------------------------------------------------------------------
PlusStatus vtkPlusSimplePublisher::SendLatestFrames(vtkPlusSimplePublisher& self, double& elapsedTimeSinceLastPacketSentSec)
{
...
PlusStatus vtkPlusSimplePublisher::SendTrackedFrame(PlusTrackedFrame& trackedFrame)
{
  ...
  else if (PublisherMessageType == "IMAGE")
  {
    for (auto& imageStream : ImageStreams)
    {
      ...
      //imageMessage.setOrigin(); TODO: convertion from igtl::Matrix4x4 to simple::Transform and simple::Transform to simple::Pose

      const uint8_t* vtkImagePointer = static_cast<const uint8_t*>(frameImage->GetScalarPointer());
      const int imageSize = imageSizePixels[0] * imageSizePixels[1] * imageSizePixels[2] * sizeof(uint8_t); // TODO: check
